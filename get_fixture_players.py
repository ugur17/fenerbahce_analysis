# This script retrieves player statistics for each fixture of Fenerbahçe from the API-Football for specified seasons and saves the raw JSON responses to files.
# It uses the requests library to make API calls and the json library to handle JSON data.
# The script also includes a delay between API calls to respect rate limits and checks for existing files
# to avoid unnecessary API calls. If a file exists but contains errors or is empty, it will be replaced with a new API call.
# Author: Ceyhun Ugur
...
for season in SEASONS:
    ...
    output_folder = f"data/raw/player_stats/{season}"
    ...
    for fixture_id in fixture_ids:
        output_file = f"{output_folder}/fixture_{fixture_id}_players.json"
